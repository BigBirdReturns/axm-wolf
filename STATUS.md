...
- The optional Glass Onion v0.3 adapter serves friendly `/wolf/SUR##` interviews, synchronizes local records through D1, authenticates operators with Cloudflare Access email codes, isolates each operator to assigned workspaces, and carries operator-uploaded analysis returns back to the participant page without runtime AI calls.
