...
- **The pack has to absorb the absence of follow-up.** A trained interviewer probes when an answer is thin. Wolf cannot. The pack must therefore include the follow-up *as its own prompt*, not assume one. The `suggestedFollowUp` field in the prompt schema carries an authored probe for exactly these moments; the capture UI surfaces it as an opt-in "Go deeper" cue beneath the prompt, so a self-directed subject can choose to follow it without being pushed. Pack authors should write a `suggestedFollowUp` on the prompts most likely to return a thin first answer.
